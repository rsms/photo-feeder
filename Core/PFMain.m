...
- (void) providerQueueFillerThread:(id)_providerAndProviderIndex
{
	...
	@try
	{
		...
		if(im)
		{
			im = [self resizeImageIfNeeded:im];
			[queue put:[self fastImageFromNSImage: im]];
		}
		else
		{
...
/// @author John Pannell
- (CVPixelBufferRef)fastImageFromCGImage:(CGImageRef)image
{
	...
	CGBitmapInfo bi = kCGImageAlphaNoneSkipFirst; // CGImageGetBitmapInfo(image);
	...
	// draw
	CGContextDrawImage(ctxt, imageRect, image);
